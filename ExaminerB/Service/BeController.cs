...
namespace ExaminerB.Backend
    ...
    public class BeController : Controller
        ...
        [HttpPost ("Read_ExamSessionNotes")]
        public async Task<ActionResult<StudentExam>> Read_ExamSessionNotes ([FromBody] int studentExamId, [FromQuery] bool excludeReadNotes)
            {
            var result = await _BeService.Read_ExamSessionNotesAsync (studentExamId, excludeReadNotes);
            return Ok (result);
            }
        ...
        public async Task<ActionResult<List<StudentExamTest>>> Read_StudentsExamTest ([FromQuery] int examId, [FromBody] int testId)
            {
            var result = await _BeService.Read_StudentsExamTestAsync (examId, testId);
            return Ok (result);
            }
